	...
	<div id="comment-<?php comment_ID(); ?>" class="comment_container">
		...
		<div class="comment-text">
			...
			<?php if ( get_option('woocommerce_enable_review_rating') == 'yes' ) : ?>
				...
				<div class="star-rating-container individual">
			...
			<?php if ($GLOBALS['comment']->comment_approved == '0') : ?>
				<p class="meta"><em><?php _e( 'Your comment is awaiting approval', '__x__' ); ?></em></p>
			<?php else : ?>
				<p class="meta">
					<strong itemprop="author"><?php comment_author(); ?></strong> <?php
						...
						if ( get_option('woocommerce_review_rating_verification_label') == 'yes' )
							if ( woocommerce_customer_bought_product( $GLOBALS['comment']->comment_author_email, $GLOBALS['comment']->user_id, $post->ID ) )
								echo '<em class="verified">(' . __( 'verified owner', '__x__' ) . ')</em> ';

					?>&ndash; <time itemprop="datePublished" datetime="<?php echo get_comment_date('c'); ?>"><?php echo get_comment_date(__( get_option('date_format'), '__x__' )); ?></time>:
				</p>
			<?php endif; ?>
