...
function x_shortcode_button( $atts, $content = null ) {
  ...
  if ( $circle == 'true' ) {
    $output = "<div {$id} class=\"x-btn-circle-wrap{$class}{$size}{$block}{$float}\" {$style}><a class=\"x-btn{$type}{$shape}{$size}{$block}{$icon_only}\" href=\"{$href}\" {$title} {$target} {$info} {$info_place} {$info_trigger} {$info_content} {$lightbox_caption} {$lightbox_options}>" . do_shortcode( $content ) . "</a></div>";
  } else {
...
function x_shortcode_responsive_video_player( $atts ) { // 1
  ...
  if ( is_numeric( $poster ) ) {
    $poster_info = wp_get_attachment_image_src( $poster, 'entry-full-' . x_get_stack() );
...
function x_shortcode_promo( $atts, $content = null ) {
  ...
  if ( is_numeric( $image ) ) {
    $image_info = wp_get_attachment_image_src( $image, 'entry-full-' . x_get_stack() );
    $image      = $image_info[0];
  }
...
function x_shortcode_post_author( $atts ) {
  ...
  $facebook_output   = ( $facebook )   ? "<a href=\"{$facebook}\" class=\"x-author-social\" title=\"Visit the Facebook Profile for {$nickname}\" target=\"_blank\"><i class=\"x-social-facebook\"></i> Facebook</a>" : '';
  $twitter_output    = ( $twitter )    ? "<a href=\"{$twitter}\" class=\"x-author-social\" title=\"Visit the Twitter Profile for {$nickname}\" target=\"_blank\"><i class=\"x-social-twitter\"></i> Twitter</a>" : '';
  $googleplus_output = ( $googleplus ) ? "<a href=\"{$googleplus}\" class=\"x-author-social\" title=\"Visit the Google+ Profile for {$nickname}\" target=\"_blank\"><i class=\"x-social-google-plus\"></i> Google+</a>" : '';

  $output = "<div {$id} class=\"{$class}\" {$style}>"
            . '<h6 class="h-about-the-author">' . $title . '</h6>'
            . get_avatar( get_the_author_meta( 'ID' ), 180 )
            . '<div class="x-author-info">'
              . "<h4 class=\"h-author mtn\">{$nickname}</h4>"
                . $facebook_output
                . $twitter_output
                . $googleplus_output
              . '<p class="p-author mbn">' . get_the_author_meta( 'description' ) . '</p>'
            . '</div>'
          . '</div>';
...
function x_shortcode_content_band( $atts, $content = null ) {
  ...
  if ( is_numeric( $bg_video_poster ) ) {
    $bg_video_poster_info = wp_get_attachment_image_src( $bg_video_poster, 'entry-full-' . x_get_stack() );
    $bg_video_poster      = $bg_video_poster_info[0];
  }

  if ( is_numeric( $bg_image ) ) {
    $bg_image_info = wp_get_attachment_image_src( $bg_image, 'entry-full-' . x_get_stack() );
    $bg_image      = $bg_image_info[0];
  }

  if ( is_numeric( $bg_pattern ) ) {
    $bg_pattern_info = wp_get_attachment_image_src( $bg_pattern, 'entry-full-' . x_get_stack() );
  ...
  if ( $bg_video != '' ) {
    ...
    $output = "<div id=\"x-content-band-{$count}\" class=\"{$class}{$bg_image_class}{$parallax_class}{$border}{$no_margin}\" style=\"background-image: url({$bg_image}); background-color: {$bg_color};{$padding_top}{$padding_bottom}{$style}\">"
              . $container_start . do_shortcode( $content ) . $container_end
            . '</div>'
            . "<script> if ( jQuery('html').hasClass('ie8') ) { jQuery('#x-content-band-{$count}').backstretch('{$bg_image}'); } </script>";
